...
void Branch::draw(ofColor _treeColor){
    ...
    //float branchNoise = ofNoise( branchCount, ofGetElapsedTimef() * 0.5 );
    //float parentNoise = ofNoise( branchCount - 1, ofGetElapsedTimef() * 0.5 );
    branchNoise *= 0.65;
    parentNoise *= 0.65;
...
void Branch::bloom(ofVec3f bloomPoint){

    if (bloomset == false){
    ofColor bloomColor = ofColor(255,ofRandom(190,210),ofRandom(200,230));
    for (int i = 0; i < 6; i++){
    Petal p;
