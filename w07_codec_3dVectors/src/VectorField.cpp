    ...
    //for (int z=0; z < field.size(); z++){
        for (int x=0; x < table.size(); x++){
            for (int y=0; y < table[x].size(); y++){
                ...
                ofLine(ofPoint(0,0,0),table[x][y]);
                ofPopMatrix();
            }
        }
    //}
...
void VectorField::setNoise(int zloc){
    for (int x=0; x < table.size(); x++){
        for (int y=0; y < table[x].size(); y++){
                float noise = ofMap(ofNoise(x*zloc*0.5,y*zloc*0.5),0,1,0,TWO_PI);
                table[x][y]=ofPoint(sin(noise),cos(noise),cos(noise));
