...
Particle::Particle() {
    ...
    splash = ofMap(pos.x,0,ofGetWidth(),0,1);
}
...
void Particle::setInit(ofPoint _pos, ofPoint _vel){
    ...
    //tailVary = ofRandomf()*50;

    if (ofRandomuf() < 0.45) {
    tailVary = ofColor(28,50,87);
    } else {
    tailVary = ofColor(42,89,119);
    }

    tailWidth = ofRandom(3,10);
...
void Particle::addWaveForce(float _heading){
    float waveSpeed;
...
if (ofGetFrameNum()%5 == 1) {
        Tail newTail;
        newTail.setInit(ofPoint(pos.x,pos.y),tailVary,tailWidth),
...
void Particle::draw() {
    ...
    for (int i = 0; i < tail.size(); i++){
        ...
        tail[i].draw(ofRandom(20),tail.size()-i);
     //ofPopMatrix();
    }


    ofSetColor(247,242,220);
    for (int i = 0; i < 10; i++) {
    ofPoint randBub = ofPoint(ofRandom(-10,10),ofRandom(-10,10));
    ofCircle(pos+randBub, radius*ofRandomf()*2);
    }
