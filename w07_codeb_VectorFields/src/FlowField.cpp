...
void FlowField::setRandomModA(float _randMin, float _randMax) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set( 1, ofRandom(_randMin,_randMax));
        }
    }
}
...
void FlowField::setCustomA(float _randMin, float _randMax) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofPushMatrix();
            ofTranslate(ofRandomWidth(),ofRandomHeight());
            //float noise = ofMap( ofNoise(x*0.05, y*0.05), 0, 1, 0, TWO_PI);
            //flowList[y][x].set( ofVec2f(abs(cos(x)*10),sin(y)*0.5) );
            flowList[y][x].set( ofVec2f(cos(x*_randMin)*10,sin(y*_randMax)*10) );
        }
    }
}

void FlowField::setCustomB(float _randMin, float _randMax) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofPushMatrix();
            ofTranslate(ofRandomWidth(),ofRandomHeight());
            //float noise = ofMap( ofNoise(x*0.05, y*0.05), 0, 1, 0, TWO_PI);
            //flowList[y][x].set( ofVec2f(abs(cos(x)*10),sin(y)*0.5) );
            flowList[y][x].set( ofVec2f(1,sin(x*0.5)*sin(y*0.5)) );
        }
    }
}

void FlowField::setCustomC(float _randMin, float _randMax) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofPushMatrix();
            ofTranslate(ofRandomWidth(),ofRandomHeight());
            //float noise = ofMap( ofNoise(x*0.05, y*0.05), 0, 1, 0, TWO_PI);
            //flowList[y][x].set( ofVec2f(abs(cos(x)*10),sin(y)*0.5) );
            flowList[y][x].set( ofVec2f(1,sin(x*0.5)*abs(sin(y*ofRandomf())*0.7)) );
        }
    }
}
